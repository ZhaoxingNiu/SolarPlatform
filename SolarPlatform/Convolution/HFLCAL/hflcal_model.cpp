...
void hflcal_model(
	...
	while (abs(sigma_high - sigma_low) > 0.01f) {
		...
		conv_method_kernel_HFLCAL(solar_scene, model_scene,rece_index, helio_index, grid_index,
			sigma_2);
...
void hflcal_model_focus(
	...
	while (abs(sigma_high - sigma_low) > 0.01f) {
		...
		sigma_2 = (sigma_high + sigma_low) / 2.0f;
		solar_scene->receivers[rece_index]->Cclean_image_content();
		conv_method_kernel_HFLCAL_focus(solar_scene, model_scene, rece_index, helio_index, sub_num, grid_index,
			sigma_2);
		float max_val = solar_scene->receivers[rece_index]->peek_value();
		if (abs(max_val - ideal_peak) < 0.1f) {
			// break
...
bool test_hflcal_model_scene1() {
	...
	for (int helio_index = 0; helio_index < 40; ++helio_index) {
		...
		// hfalcal model
		hflcal_model(solar_scene, model_scene,rece_index, helio_index, grid_index, ideal_peak, res_path);
	}
...
bool test_hflcal_model_ps10() {
	...
	for (int helio_index = 0; helio_index < helio_index_range; ++helio_index) {
		...
		float ideal_peak = get_file_peak(raytracing_path);
		// hfalcal model
		hflcal_model_focus(solar_scene, model_scene, rece_index, helio_index, 28, 0 ,ideal_peak, res_path);
	}
	std::cout << "程序平均耗时：" << solarenergy::total_time / solarenergy::total_times << " ms" << endl;
	return true;
}