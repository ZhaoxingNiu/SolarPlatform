...
const DoubleArray csaps::SolveLinearSystem(const DoubleSparseMatrix &A, DoubleArray &b)
{
  ...
  auto tmp = b.matrix();
  DoubleArray x = solver.solve(tmp).array();
  return x;
}
...
void UnivariateCubicSmoothingSpline::InitEdge() {
	const Size x_size = m_xdata.size();
	DoubleArray mesh = m_xdata.segment(1, x_size - 2);
	m_edges.resize(x_size);
	m_edges(0) = -DoubleLimits::infinity();
	m_edges.segment(1, x_size - 2) = mesh;
	m_edges(x_size - 1) = DoubleLimits::infinity();
}


DoubleArray UnivariateCubicSmoothingSpline::Evaluate(const DoubleArray & xidata, int deriv)
{
  IndexArray indexes = Digitize(xidata, m_edges);
  ...
  DoubleArray yidata_res(xi_size);
  ...
  for (Index i = 0; i < xi_size; ++i) {
    ...
    // Initial values
	double y = m_coeffs(index, 0);
    yidata(i) = m_coeffs(index, 0);
  }

  DoubleArray coeffs(xi_size);

  for (Index i = 1; i < m_coeffs.cols()- deriv; ++i) {
...
double UnivariateCubicSmoothingSpline::Evaluate(const double &x_val, int deriv){
	DoubleArray xidata(1);
	xidata << x_val;
	DoubleArray yidata = Evaluate(xidata, deriv);
	return yidata(0);
}
